...
import com.example.Alexandria.models.MineUserInfo
...
//        Обновление username в БД для текущего пользователя
    REF_DATABASE_ROOT.child(NODE_USERS).child(
        CURRENT_UID
...
//        Удаление старого username
    REF_DATABASE_ROOT.child(NODE_USERNAMES).child(
        USER1.userName).removeValue()
        .addOnSuccessListener {
                showToast(
                    APP_ACTIVITY.getString(
                        R.string.toast_data_update
